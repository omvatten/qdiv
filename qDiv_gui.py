...
def Consensus_object(path):
    ...
    def consensusObj():
        ...
        cons, info = makeConsensusObject(objlist, differentLengths=differentLengths)
...
def Subsetting(obj, path):
    ...
    def subset_top_svs():
        ...
        returnFiles(obj_sub, path=path, sep=',', savename='TopSeqs')
    ...
    def merge_smps():
        ...
        returnFiles(obj_sub, path=path, sep=',', savename='Merged')
...
def Mantel_test():
    ...
    def run_mantel():
        ...
        if methodname == 'mean absolute distance':
            methodname = 'absDist'
        res = mantel(dis1, dis2, method=methodname, permutations=perm_nr.get())
...
def startwindow():
    ...
    tk.Button(root, text='Press for info', command=stats).grid(row=20, column=0, sticky=tk.W)
